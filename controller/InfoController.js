...
const getInfo = async (req, res,next) => {
    ...
    res.status(200).json({ Info: Info.map(Info => Info.toObject({ getters: true })) });
  };

//creating a new Info
const createInfo = async (req,res,next) => {
    try {
        let { error } = InfoValidate(req.body);
        ...
        const error = new HttpError(
          'Creating New Information Failed.'
        );
        return next(error);
      }
};
const PutInfo = async (req,res,next)=>{
  try {
      let Info= await InfoModel.findByIdAndUpdate(req.params.id,req.body,{new:true});
